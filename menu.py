...
class SettingsMenu(Entity):
    def __init__(self,music,volume_func,on_leave=lambda: print("leaving"), add_to_scene_entities=True, **kwargs):
        ...
        self.volume_slider = Slider(min=0, max=1, step=0.01, default=music.volume, dynamic=True, position=(-0.5,-0.2),parent=self, text='Volume', text_origin=(-.3,0), text_scale=0.5, text_offset=(-.1,0), on_value_changed=volume_func,bar_color=color.white)
        self.close_button = Button(model="quad",texture="closeButton",scale=Vec2(.03)*3,position=(-.5,.1),on_click=self.leave,parent=self,highlight_texture="closeButton",color=color.white,highlight_color=color.gray)
...
if __name__ == "__main__":
    ...
    music = Audio("loop",loop=True, autoplay=True)
